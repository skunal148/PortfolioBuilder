...
function LiveVisualStorytellerEditor() {
    ...
    const handleSavePortfolio = async () => {
        ...
        let finalResumeUrl = resumeUrl; // Initialize with existing URL
        ...
        try {
            ...
            if (resumeFile) {
                ...
                try {
                    finalResumeUrl = await handleImageUpload( // Using existing handleImageUpload, ensure it's suitable or create a specific one for files
                        resumeFile,
                        `resumes_${THIS_TEMPLATE_ID}/${auth.currentUser.uid}`,
            ...
            setIsUploadingProfilePic(false);
            setIsUploadingHeroImage(false);
            setIsUploadingResume(false); // Reset resume upload state
            ...
            <div className={`live-portfolio-editor-container container mx-auto p-4 md:p-6 grid ${isMobileView ? 'grid-cols-1' : 'lg:grid-cols-2'} gap-6 items-start`}>
                <div className={`${editorPanelBg} p-4 sm:p-6 rounded-xl shadow-2xl space-y-6 ${isMobileView ? 'w-full' : 'max-h-[calc(100vh-100px)] overflow-y-auto custom-scrollbar'}`}>
                    ...
                    <div className="save-button-container mt-8">
                        <button onClick={handleSavePortfolio} disabled={loading || isUploadingProfilePic || isUploadingHeroImage || isUploadingResume || projects.some(p => p.isUploadingThumbnail)} className={`${buttonClasses} w-full text-lg py-3 disabled:opacity-70 disabled:cursor-not-allowed`}>
                            {saveButtonText}
                        </button>
                    </div>
