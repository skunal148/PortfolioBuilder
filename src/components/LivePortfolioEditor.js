...
function LivePortfolioEditor() {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    setProjects(
                        ...
                        ? data.projects.map(p => ({ ...createNewProject(), ...p, id: String(p.id || generateStableId('project')), isCollapsed: p.isCollapsed !== undefined ? p.isCollapsed : false, thumbnailFile: null, isUploadingThumbnail: false, thumbnailUploadProgress: 0 }))
                        : [createNewProject()]
                    );
                    setSkills(Array.isArray(data.skills) ? data.skills : []);
                    setCustomSections(
                        Array.isArray(data.customSections)
                        ? data.customSections.map(cs => ({ ...createNewCustomSection(), ...cs, id: String(cs.id || generateStableId('customSection')), items: Array.isArray(cs.items) ? cs.items.map(item => ({ ...createNewCustomSectionItem(), ...item, id: String(item.id || generateStableId('customItem')), isCollapsed: item.isCollapsed !== undefined ? item.isCollapsed : false })) : [] }))
                        : []
                    );
                    ...
                    // No need to set individual visibility states, Disclosure handles it
        ...
        try {
            ...
            for (let i = 0; i < finalProjects.length; i++) {
                ...
                if (project.thumbnailFile) {
                    ...
                    setProjects([...finalProjects]); // Update UI for individual upload start
                    try {
                        const newUrl = await handleImageUpload(
                            project.thumbnailFile,
                            `projectThumbnails_styled/${project.id || Date.now()}`,
                            (progress) => {
                    ...
                    finalProjects[i] = project;
                    setProjects([...finalProjects]); // Update UI with final state
            ...
            const projectsToSave = finalProjects.map(p => ({
                id: String(p.id), title: p.title, description: p.description, // description is HTML
            ...
            const customSectionsToSave = customSections.map(cs => ({
                ...
                items: cs.items.map(item => ({
                    id: String(item.id), itemTitle: item.itemTitle, itemDetails: item.itemDetails, // itemDetails is HTML
            ...
            const portfolioData = {
                ...
                linkedinUrl, githubUrl, aboutMe, // aboutMe is HTML
                ...
                // Removed individual visibility states as Disclosure handles them
    ...
    // --- Class Definitions for Editor Panel (Always Dark) ---
    ...
    if (skillChipStyleOverride === 'light') {
        finalSkillChipBgForPreview = '#E2E8F0'; finalSkillChipTextForPreview = '#2D3748';
    } else if (skillChipStyleOverride === 'dark') {
        finalSkillChipBgForPreview = '#2D3748'; finalSkillChipTextForPreview = '#E2E8F0';
    } else {
        const isDarkText = bodyTextColor.toLowerCase().startsWith('#e') || bodyTextColor.toLowerCase().startsWith('#f') || bodyTextColor.toLowerCase().startsWith('#d');
        finalSkillChipBgForPreview = isDarkText ? '#4A5568' : '#E5E7EB';
        finalSkillChipTextForPreview = isDarkText ? '#F7FAFC' : '#1F2937';
    }
        ...
        <DragDropContext onDragEnd={onDragEnd}>
            <div className={`live-portfolio-editor-container container mx-auto p-4 md:p-6 grid ${isMobileView ? 'grid-cols-1' : 'lg:grid-cols-2'} gap-6 items-start`}>
                ...
                <div className={`${editorPanelBg} p-4 sm:p-6 rounded-xl shadow-2xl space-y-6 ${isMobileView ? 'w-full' : 'max-h-[calc(100vh-100px)] overflow-y-auto custom-scrollbar'}`}>
                    ...
                    {/* Basic Info Section (Not Collapsible for Styled Editor) */}
                            ...
                            <>
                                ...
                                <Transition enter="transition duration-100 ease-out" enterFrom="transform scale-95 opacity-0 max-h-0" enterTo="transform scale-100 opacity-100 max-h-screen" leave="transition duration-75 ease-out" leaveFrom="transform scale-100 opacity-100 max-h-screen" leaveTo="transform scale-95 opacity-0 max-h-0" >
                                    <Disclosure.Panel className="skills-section mt-3 space-y-3 overflow-hidden">
                                        {/* Skills input and list structure identical to LiveBlankPortfolioEditor */}
                                                ...
                                                {(provided) => (
                                                    <ul ref={provided.innerRef} {...provided.droppableProps} className={`skills-list space-y-1 mt-2`}>
                                                        {skills.map((skillObj, index) => (
                                                            <Draggable key={`skill-styled-${skillObj.name}-${index}`} draggableId={`skill-styled-${skillObj.name}-${index}`} index={index} isDragDisabled={isMobileView}>
                                                                {(providedDraggable) => (
                                                                    <li ref={providedDraggable.innerRef} {...providedDraggable.draggableProps} className={`flex justify-between items-center ${editorDraggableItemBg} p-2 rounded text-sm ${editorDraggableItemText}`}>
                                                                        ...
                                                                        <button type="button" onClick={() => handleRemoveSkill(skillObj.name)} className="text-rose-400 hover:text-rose-300 p-1 rounded-full hover:bg-slate-600 transition-colors"> <RemoveIcon className="w-4 h-4" /> </button>
                                                                    </li>
                                                                )}
                                                            </Draggable>
                                                        ))}
                                                            ...
                                                            {(providedDraggable) => (
                                                                <div ref={providedDraggable.innerRef} {...providedDraggable.draggableProps} className={`project-item-editor-wrapper ${editorDraggableItemBg} rounded-md shadow-md`}>
                                                                    <div className="flex justify-between items-center p-3 border-b border-slate-600">
                                                                        ...
                                                                        <button type="button" onClick={() => handleToggleProjectItemCollapse(project.id)} className="p-1 text-slate-400 hover:text-slate-200 mr-2"> {project.isCollapsed ? <ChevronDownIcon /> : <ChevronDownIcon className="transform rotate-180"/>} </button> {/* ChevronUp can be a rotated ChevronDown */}
                            ...
                            <>
                                ...
                                <Transition enter="transition duration-100 ease-out" enterFrom="transform scale-95 opacity-0 max-h-0" enterTo="transform scale-100 opacity-100 max-h-screen" leave="transition duration-75 ease-out" leaveFrom="transform scale-100 opacity-100 max-h-screen" leaveTo="transform scale-95 opacity-0 max-h-0" >
                                    <Disclosure.Panel className="customization-section mt-3 space-y-6 overflow-hidden">
                                       {/* All customization inputs from LiveBlankPortfolioEditor should be here */}
                                        <div>
