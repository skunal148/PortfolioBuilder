...
// Social Icons
const LinkedInIcon = ({ fill = "#FFFFFF" }) => ( /* ... SVG ... */ 
...
const GitHubIcon = ({ fill = "#FFFFFF" }) => ( /* ... SVG ... */ 
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill={fill} className="transition-opacity hover:opacity-75">
...
const SkillDisplay = ({ skillName, fallbackTextColor, displayStyle }) => { /* ... same as before ... */ 
  ...
  switch (normalizedSkillName) {
    ...
    case 'java': case 'Java': iconComponent = <JavaIcon />; break;
  ...
  if (displayStyle === 'icon-only-chip') {
    ...
    <div className="flex flex-col items-center text-center w-full">
      {iconComponent} 
      <span className="text-s mt-1" style={{ color: fallbackTextColor }}>
        {skillName}
      </span>
    </div>
  );
  ...
  if (normalizedSkillName && (normalizedSkillName === 'react' || normalizedSkillName === 'react.js' || normalizedSkillName === 'javascript' || normalizedSkillName === 'js' || normalizedSkillName === 'python' || normalizedSkillName === 'html' || normalizedSkillName === 'html5' || normalizedSkillName === 'css' || normalizedSkillName === 'css3')) {
    return iconComponent; 
  }
  return (
    <div className="flex flex-col items-center text-center w-full">
...
const getMarginBottomClass = (spacingValue) => { /* ... same as before ... */ 
    const spacingMap = ['mb-0', 'mb-2', 'mb-4', 'mb-6', 'mb-8', 'mb-12', 'mb-16', 'mb-20', 'mb-24'];
    const index = Math.max(0, Math.min(spacingValue, spacingMap.length - 1));
...
function PortfolioDisplay({ portfolioData }) {
  ...
  // ... (background style logic remains the same)
  if (templateId && templateId !== 'blank') {
    ...
    if (backgroundType === 'theme') {
        const currentTheme = predefinedBackgroundThemes.find(t => t.id === selectedBackgroundTheme);
        livePreviewBackgroundStyle = currentTheme ? currentTheme.style : predefinedBackgroundThemes[0].style;
  ...
  const basePreviewClasses = "shadow-xl rounded-xl p-6 md:p-8 overflow-y-auto";
  ...
  const renderHeader = () => { /* ... same as before ... */ 
    const socialLinks = (
        <div className={`social-links flex items-center space-x-4 mt-2 ${headerLayout === 'image-top-center' || headerLayout === 'text-only-center' ? 'justify-center' : 'justify-start'}`}>
            {linkedinUrl && <a href={linkedinUrl} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn Profile"><LinkedInIcon fill={accentColor} /></a>}
            {githubUrl && <a href={githubUrl} target="_blank" rel="noopener noreferrer" aria-label="GitHub Profile"><GitHubIcon fill={accentColor} /></a>}
        </div>
    );
    const textContent = (
        <div className={`${headerLayout === 'image-left-text-right' ? 'text-left' : 'text-center'}`}>
            {name && <h1 className={`text-3xl md:text-4xl font-bold mb-1 ${headerLayout === 'image-left-text-right' ? '' : 'mx-auto'}`} style={{color: headingColor}}>{name}</h1>}
    ...
    if (headerLayout === 'image-left-text-right') {
        return (
            <div className="flex flex-col md:flex-row items-center md:items-start text-left">
                {profilePicture && <img src={profilePicture} alt="Profile" className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover mb-4 md:mb-0 md:mr-8 border-4 border-slate-500 shadow-lg flex-shrink-0" />}
    ...
    return (
        <div className="text-center">
            {profilePicture && <img src={profilePicture} alt="Profile" className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover mx-auto mb-4 border-4 border-slate-500 shadow-lg" />}
            {textContent}
        </div>
    );
  };
  ...
  return (
    ...
    >
      {(name || profilePicture || linkedinUrl || githubUrl) && (
        <div className={`portfolio-header pb-6 border-b border-slate-600 ${sectionMarginClass}`}>
          {renderHeader()}
        </div>
      )}
      {aboutMe && (
        <div className={`portfolio-about ${sectionMarginClass}`}>
          <h2 className="text-2xl font-semibold mb-3" style={{color: headingColor}}>About Me</h2>
          <p className="text-base leading-relaxed whitespace-pre-line" style={{color: bodyTextColor }}>{aboutMe}</p>
        </div>
      )}
      {skills.length > 0 && (
        <div className={`portfolio-skills ${sectionMarginClass}`}>
          <h2 className="text-2xl font-semibold mb-3" style={{color: headingColor}}>Skills</h2>
          {skillDisplayStyle === 'text-only-list' ? (
            <ul className="list-disc list-inside pl-5 space-y-1">
              {skills.map((skill, index) => (
                <li key={index} style={{ color: bodyTextColor }}>{skill}</li>
              ))}
            </ul>
          ) : ( 
            <ul className="flex flex-wrap gap-3 items-center"> 
              {skills.map((skill, index) => (
                <li 
                  key={index} 
                  className={`flex items-center justify-center p-2.5 rounded-lg shadow-md transition-colors duration-150 
                              ${skillDisplayStyle === 'icon-only-chip' ? 'bg-slate-700/50 hover:bg-slate-600/50' : 'bg-slate-600/70 hover:bg-slate-500/50'}`}
                  title={skill} 
                >
                  <SkillDisplay skillName={skill} fallbackTextColor={bodyTextColor} displayStyle={skillDisplayStyle} />
                </li>
              ))}
            </ul>
          )}
      ...
      {/* MODIFIED: Projects Section to display thumbnails AND URLs */}
      {projects.some(p => p.title || p.description || p.thumbnailUrl || p.liveDemoUrl || p.sourceCodeUrl || p.videoUrl) && (
        <div className={`portfolio-projects ${skills.length === 0 && aboutMe ? sectionMarginClass : (skills.length === 0 && !aboutMe && (name || profilePicture || linkedinUrl || githubUrl) ? sectionMarginClass : '')}`}>
          <h2 className="text-2xl font-semibold mb-4" style={{color: headingColor}}>Projects</h2>
          ...
          {projects.map((project, index) => (
            ...
            <div key={project.id || `project-preview-${index}`} className="portfolio-project bg-slate-600/50 p-4 rounded-lg shadow"> 
              ...
              {project.title && <h3 className="text-xl font-bold mb-1" style={{color: headingColor}}>{project.title}</h3>}
              {project.description && <p className="text-sm whitespace-pre-line mb-3" style={{color: bodyTextColor }}>{project.description}</p>}
              
              {/* MODIFIED: Display Video Embed if videoUrl exists */}
              {project.videoUrl && (
              ...
              {/* MODIFIED: Display Live Demo and Source Code Links */}
              <div className="flex space-x-4 mt-3">
                {project.liveDemoUrl && (
                  <a 
                    ...
                    className="inline-block bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold py-2 px-3 rounded-md transition-colors"
                    style={{backgroundColor: accentColor}} // Use accentColor for buttons
                  >
                    Live Demo
                  </a>
                )}
      ...
      {!name && !profilePicture && !aboutMe && skills.length === 0 && !projects.some(p => p.title || p.description) && (
        <p className="text-center text-slate-400 text-lg py-10">This portfolio is currently empty.</p>
      )}
    </div>
  );
}
...
const getEmbedUrl = (url) => {
    ...
    // If no match, return empty or the original URL (which might not embed)
    return ''; // Or consider returning the original URL if you want to try embedding other things, but it's less reliable
};
